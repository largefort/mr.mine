...
const TRADE_TYPE_BUILDING_MATERIAL = 5;
...
const tradeConfig = {
    ...
    duration: 3600,                     // Duration trade is available in seconds
    medianValueGain: 1.4,               // Median time saved for completing a trade (e.g., 1 hour of coal yields 2 hours of gold)
    valueSpread: 1.5,                   // Std dev for value gain calculation
    medianTimeBetweenTrades: 10800,     // Median time after the current trade ends that the next one will be generated
...
{
    return 3600 * tradingPostStructures[worldIndex].statValueForCurrentLevel();
}
...
function generateTradeOffer(trader, tradeOfferArray, worldIndex)
{
    ...
    var tradeAmounts = determineTradeAmounts(paymentType, paymentSubtype, rewardType, rewardSubtype);
    if(tradeAmounts == null)
    {
        return null;
    }
    tradeOfferArray[TRADE_INDEX_START_TIME] = currentTime() / 1000;
    tradeOfferArray[TRADE_INDEX_DURATION] = tradeConfig.duration;
        ...
        switch(paymentType.type)
        {
            case TRADE_TYPE_ORE:
                if(!paymentType.subtypes)
                {
                    ...
                    {
                        //AO: change the below to use a random mineral of last 3 discovered for world

                        var randomOreIndex = tradeRoller.rand(0, paymentSubtypes.length - 1);

                        var randomOreId = paymentSubtypes[randomOreIndex];
                ...
                else
                {
                    ...
                    // Trader has hardcoded specified mineral types
        ...
        if(!disallowMoney || rewardType.type != TRADE_TYPE_MONEY)
        {
            switch(rewardType.type)
            {
                ...
                case TRADE_TYPE_BUFF:
                    var buffIndex = tradeRoller.rand(0, buffs.staticBuffs.length - 1);
                    rewardSubtype = buffIndex;
                    break;
                case TRADE_TYPE_ORE:
                ...
                case TRADE_TYPE_MONEY:
                    rewardSubtype = 0;
                    break;
                case TRADE_TYPE_BUILDING_MATERIAL:
                    rewardSubtype = 0;
                    break;
                default:
                    return null;
...
function determineTradeAmounts(paymentType, paymentSubtype, rewardType, rewardSubtype)
{
    var paymentTimeValue, rewardTimeValue;
    var paymentAmount, rewardAmount;
    var valueGain = gaussianRand(tradeConfig.medianValueGain, tradeConfig.valueSpread);
...
{
    var value1 = valueOfMineralsPerSecond() * (BigInt(paymentTimeValue) / BigInt(valueGain));
    var value2 = rewardSubtypeValue * valueFraction;
    var value3 = bigIntMax(money.add(getValueOfMinerals()), BigInt(value2)) / BigInt(2);
...
function determineTimeValueOfTradeType(tradeType, tradeSubtypeIndex = null)
{
    ...
    var resultTimeValue = BigInt(0);
    switch(tradeType)
    {
        case TRADE_TYPE_ORE:
            resultTimeValue = BigInt(3600);
            break;
        case TRADE_TYPE_CHEST:
            if(tradeSubtypeIndex == 0)
            {
                // Basic chest
                resultTimeValue = BigInt(600);
            }
            else
            {
                // Gold chest
                resultTimeValue = BigInt(7200);
            }
            break;
        case TRADE_TYPE_MONEY:
            resultTimeValue = BigInt(3600);
            break;
        case TRADE_TYPE_BLUEPRINT:
            resultTimeValue = BigInt(14400);
            break;
        case TRADE_TYPE_BUFF:
            resultTimeValue = BigInt(900);
            break;
        case TRADE_TYPE_RELIC:
            if(tradeSubtypeIndex)
            {
                resultTimeValue =  doBigIntDecimalMultiplication(BigInt(43200), Math.pow(excavationRewards[tradeSubtypeIndex].rarity, 2.5));
            }
            break;
        case TRADE_TYPE_BUILDING_MATERIAL:
            resultTimeValue = BigInt(600);
...
function calculateAmountOfTradeType(tradeType, tradeSubtype, timeValue, isPayment, otherType = null, otherSubtype = null, otherAmount = null)
{
    ...
    switch(tradeType)
    {
        ...
        case TRADE_TYPE_ORE:
            ...
            {
                var otherValue = BigInt(otherAmount) * worldResources[otherSubtype].sellValue;
                var quantity = (((BigInt(timeValue) * BigInt(100)) / determineTimeValueOfTradeType(TRADE_TYPE_ORE)) * otherValue / worldResources[tradeSubtype].sellValue) / BigInt(100);
                if(quantity <= BigInt(0)) quantity = BigInt(1);
                return quantity;
            }
            ...
            {
                var mineralCounts = estimateTotalMineralsMinedPerSecond();
                return doBigIntDecimalMultiplication(BigInt(timeValue), mineralCounts[tradeSubtype]); //amount of a mineral made in time (timeValue) as an input for trade
            }
        case TRADE_TYPE_MONEY:
            if(!isPayment && otherType == TRADE_TYPE_ORE)
            {
                var mineralCounts = estimateTotalMineralsMinedPerSecond();
                var count = doBigIntDecimalMultiplication(BigInt(timeValue), mineralCounts[otherSubtype]);
                return count * worldResources[otherSubtype].sellValue;
            }
            else if(otherType == TRADE_TYPE_BUILDING_MATERIAL)
            {
                return BigInt(timeValue) * valueOfMineralsPerSecond();
            }
            else
            {
        ...
        case TRADE_TYPE_BUILDING_MATERIAL:
            if(!isPayment && otherType == TRADE_TYPE_ORE)
            {
                //this is the amount to reward in BMs
                return timeValue / BigInt(3600);
            }
            else
            {
                //amount to sell in BMs
                return Math.min(25, Math.max(1, Math.floor(numBuildingMaterialsOwned()/10)));
            }
        default:
            throw "Invalid trade type";
    }
...
function canMakeTrade(tradeOffer, showMessageOnFail = false)
{
    ...
    if(!canMakeTrade && showMessageOnFail)
    {
        newNews(_(message));
    }
    return canMakeTrade;
...
function makeTrade(worldIndex, tradeOffer)
{
    ...
    switch(tradeOffer[TRADE_INDEX_REWARD_TYPE])
    {
        ...
        case TRADE_TYPE_BUFF:
            ...
            {
                buffs.startBuff(tradeOffer[TRADE_INDEX_REWARD_SUBTYPE], 300, "trade", 50);
            }
            else
            {
...
function setFirstTrade()
{
    ...
    tradeArrays[1][TRADE_INDEX_PAYMENT_AMOUNT] = BigInt(2000);
    tradeArrays[1][TRADE_INDEX_REWARD_TYPE] = TRADE_TYPE_CHEST;
    tradeArrays[1][TRADE_INDEX_REWARD_SUBTYPE] = 0;
    tradeArrays[1][TRADE_INDEX_REWARD_AMOUNT] = BigInt(1);
