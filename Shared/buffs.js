...
class Buffs
{
    ...
    update()
    {
        ...
        if(!this.checkedForMobileBuff)
        {
            let mobileBuffIndex = this.activeBuffs.findIndex(buff => buff.startSource == "Ad");

            if(mobileBuffIndex < 0 && isMobile())
            {
                this.startBuff(10, 60 * 60 * 8, "Ad", 100);
                this.activeBuffs.find(buff => buff.startSource == "Ad").millisecondsRemaining = 0;

            }

            this.checkedForMobileBuff = true;
        }
    }
    ...
    getBuffValue(statId)
    {
        ...
        {
            const buff = this.activeBuffs[i];
            if(buff.stat.id == statId && buff.millisecondsRemaining > 0 && !buff.isPaused())
            {
...
newStaticBuff.isPaused = () => !isMobile() || typeof adManager == "undefined" || !adManager.isReady();
buffs.registerStaticBuff(newStaticBuff);