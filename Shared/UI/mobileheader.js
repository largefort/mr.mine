class MobileHeader extends Hitbox
{
    ...
    init()
    {
        ...
        var barAreaHeight = this.boundingBox.height * (1 - this.topPadding);
        var bigBarWidth = Math.ceil(this.boundingBox.width * 0.293);
        var bigBarHeight = barAreaHeight * 0.35;
        this.barHorizontalPadding = this.boundingBox.width * 0.04;
        var bigBarVerticalPadding = this.boundingBox.height * this.topPadding;

        var smallBarWidth = this.boundingBox.width * 0.21;
        var smallBarHeight = barAreaHeight * 0.3;
        var smallBarVerticalPadding = barAreaHeight * 0.08;
        ...
        {
            if(money.greaterThanOrEqualTo(1000000))
            {
        ...
        bar.getFillPercent = () => Math.min(1, divideBigNumberToDecimalNumber(progressTowardsNextDepth, depthDifficultyTable[depth]));
        ...
        bar.getFillPercent = () => 0;
        bar.getText = () => tickets;
        bar.setIcon(smallShopTicketGold, -0.05);
    ...
    onmouseexit()
    {
        if(!(currentTargetHitbox && currentTargetHitbox.hitbox && this.isAncestorOf(currentTargetHitbox.hitbox)))
        {
