class ChestCollectorWindow extends BottomTabbedPopup
{
    ...
    constructor(boundingBox)
    {
        ...
        let tabs = chestCompressorStructure.level > 0 ? {0: _("Compress"), 1: _("Queue")} : {};
        this.initializeTabs(Object.values(tabs));
    ...
    initializeBaseTab()
    {
        ...
        if(chestCompressorStructure.level > 0)
        {
            this.compressorPane.basicChestCompressButton = this.compressorPane.addHitbox(new Button(
                ...
                {
                    x: this.compressorPane.basicChestDisplay.boundingBox.x,
                    y: this.compressorPane.basicChestDisplay.boundingBox.height * .9,
                    width: this.compressorPane.basicChestDisplay.boundingBox.width,
                    height: this.compressorPane.basicChestDisplay.boundingBox.height * 0.15
                },
                {
        ...
        if(chestCompressorStructure.level > 0)
        {
            this.compressorPane.goldChestCompressButton = this.compressorPane.addHitbox(new Button(
                ...
                {
                    x: this.compressorPane.goldChestDisplay.boundingBox.x,
                    y: this.compressorPane.goldChestDisplay.boundingBox.height * .9,
                    width: this.compressorPane.goldChestDisplay.boundingBox.width,
                    height: this.compressorPane.goldChestDisplay.boundingBox.height * 0.15
                },
                {
        ...
        else if(this.currentTabIndex == 1)
        {
            ...
            for(var i = 0; i < chestCompressor.queuedChests.length; i++)
            {
                ...
                if(hitbox)
                {
                    ...
                    this.queuePane.context.drawImage(chestIcon, 0, 0, chestIcon.width, chestIcon.height, hitbox.boundingBox.x, hitbox.boundingBox.y + (hitbox.boundingBox.height * 0.05), relativeWidth(chestIcon, 720) * .33, relativeHeight(chestIcon, 1280) * .33)
                    ...
                    this.queuePane.context.drawImage(bigbutton, 0, 0, bigbutton.width, bigbutton.height, button.boundingBox.x, button.boundingBox.y, button.boundingBox.width, button.boundingBox.height)
