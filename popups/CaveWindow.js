class CaveWindow extends BottomTabbedPopup
{
    ...
    initializeDroneList()
    {
        ...
        var extendButton = this.droneListScrollbox.addHitbox(new Hitbox(
            {
                x: this.boundingBox.width * 0.25,
                y: this.droneListBar.boundingBox.height * 0.15,
                width: this.boundingBox.width * 0.5,
                height: this.droneListBar.boundingBox.height * 0.7
            },
            {
                onmousedown: function ()
                {
                    showAdPrompt(
                        _("Pay 1 ticket or watch an ad to extend the cave duration by 30 minutes"),
                        1,
                        function()
                        {
        ...
        for(var i = 0; i < 20; ++i)
        {
            var droneDisplayHitbox = new Hitbox(
                {
                    x: 0,
                    y: (this.droneListScrollbox.boundingBox.height * 0.015) + (i + 1) * this.droneListBar.boundingBox.height,
            ...
            // this.droneListScrollbox.contentWidth - barHeight,
            // lineYCoordinate + barHeight / 2 - 0.325 * barHeight,
            // barHeight * 0.65,
            // barHeight * 0.65

            var returnButton = droneDisplayHitbox.addHitbox(new Hitbox(
                {
                    x: this.droneListScrollbox.contentWidth - droneDisplayHitbox.boundingBox.height,
                    y: droneDisplayHitbox.boundingBox.height * (1 - 0.65) / 2,
                    width: droneDisplayHitbox.boundingBox.height * 0.65,
                    height: droneDisplayHitbox.boundingBox.height * 0.65,
                },
                {
                    onmousedown: function (i)
                    {
            ...
            returnButton.isEnabled = function(droneIndex) { 
                return this.caveSystem.activeDrones[droneIndex] && this.caveSystem.activeDrones[droneIndex].isMovingForward; 
            }.bind(this, i);
        }
    }
    ...
    renderDroneList()
    {
        ...
        context.font = this.droneListBar.boundingBox.height * 0.5 + "px KanitB";
        context.fillStyle = "#000000";
        context.textBaseline = "middle";
        ...
        drawNineSlice(
            context,
            adButton,
            this.boundingBox.width * 0.25,
            this.droneListBar.boundingBox.height * 0.15,
            this.boundingBox.width * 0.5,
            this.droneListBar.boundingBox.height * 0.7,
            31,
            24
        );
        ...
        context.drawImage(
            ...
            this.boundingBox.width * 0.3, 
            this.droneListBar.boundingBox.height * 0.275, 
            20, 
            20
            );
        ...
        fillTextShrinkToFit(
            ...
            _("Extend"),
            this.boundingBox.width * 0.25,
            this.droneListBar.boundingBox.height * 0.15 + (this.droneListBar.boundingBox.height * 0.35),
            this.boundingBox.width * 0.5,
            "center"
        );
        ...
        this.droneListScrollbox.contentHeight = Math.max(
            ...
            this.droneListBar.boundingBox.height * (this.caveSystem.activeDrones.length + 1)
        );
        ...
        for(var i = 0; i < this.caveSystem.activeDrones.length; i++)
        {
            var drone = this.caveSystem.activeDrones[i];
            context.globalAlpha = 0.3;
            lineYCoordinate = ((barHeight * 1.02) * (i + 1));
            if(i % 2 == 0)
            {
            ...
            var inventoryX = this.droneListScrollbox.boundingBox.width * 0.675;
            var inventoryWidth = this.droneListScrollbox.boundingBox.width * 0.165;
            var inventorySlotsPerRow;
            var extraRowOffset = 0;
            if (drone.rewardCapacity <= 3) 
            {
                inventorySlotsPerRow = 3;
            }
            else if (drone.rewardCapacity <= 8) 
            {
                inventorySlotsPerRow = 4;
                if (drone.rewardCapacity == 4)
                {
                    extraRowOffset = 1;
                }
            }
            ...
            {
                inventorySlotsPerRow = 5;
            }
            var inventoryRows = Math.ceil(drone.rewardCapacity / inventorySlotsPerRow);
            var inventoryBoxWidth = inventoryWidth / (inventorySlotsPerRow + 0.5);
            inventoryX += inventoryBoxWidth * 0.125;
            var inventoryBoxHPadding = (inventoryWidth - (inventorySlotsPerRow * inventoryBoxWidth)) / (inventorySlotsPerRow + 2);
            var inventoryBoxVPadding = inventoryBoxWidth * 0.05;
            var inventoryHeight = inventoryRows * inventoryBoxWidth + (inventoryRows + 1) * inventoryBoxVPadding;
            var inventoryY = lineYCoordinate + barHeight / 2 - inventoryHeight / 2;
            for(var j = 0; j < drone.rewardCapacity; ++j)
            {
                var coords = getItemCoordsInList(
                    inventoryBoxWidth, 
                    inventoryBoxWidth, 
                    inventoryBoxHPadding,
                    inventoryBoxVPadding,
                    inventorySlotsPerRow,
                    j
                );
                coords.x += inventoryX;
                coords.y += inventoryY;
